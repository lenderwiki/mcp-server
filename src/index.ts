...
server.tool(
  "find_lenders",
  "Search the LenderWiki database for consumer lenders. Filter by state, lender type, credit score acceptance, ITIN acceptance, and more. Returns a list of matching lenders with key details.",
  {
    ...
    accepts_itin: z.boolean().optional().describe("Filter for lenders that accept ITIN (Individual Taxpayer Identification Number)"),
    min_amount: z.number().optional().describe("Minimum loan amount needed — filters to lenders offering at least this amount"),
    max_amount: z.number().optional().describe("Maximum loan amount needed — filters to lenders whose minimum is at or below this amount"),
  ...
  async (params) => {
    ...
    if (params.accepts_itin) {
      query = query.eq("accepts_itin", true);
    }
    ...
    if (params.credit_score !== undefined || params.min_amount !== undefined || params.max_amount !== undefined) {
      const productIds = filtered.map(l => l.id);

      const [eligRes, prodRes] = await Promise.all([
        params.credit_score !== undefined
          ? supabase.from("product_eligibility").select("lender_id, min_credit_score").in("lender_id", productIds)
          : Promise.resolve({ data: null }),
        (params.min_amount !== undefined || params.max_amount !== undefined)
          ? supabase.from("lender_products").select("lender_id, min_amount, max_amount").in("lender_id", productIds)
          : Promise.resolve({ data: null }),
      ]);

      if (params.credit_score !== undefined && eligRes.data) {
        const lenderMinScores = new Map<string, number>();
        for (const e of eligRes.data) {
          const current = lenderMinScores.get(e.lender_id);
          if (current === undefined || (e.min_credit_score !== null && e.min_credit_score < current)) {
            lenderMinScores.set(e.lender_id, e.min_credit_score ?? 0);
          }
        }
        filtered = filtered.filter(l => {
          const minScore = lenderMinScores.get(l.id);
          return minScore === undefined || minScore <= params.credit_score!;
        });
      }

      if ((params.min_amount !== undefined || params.max_amount !== undefined) && prodRes.data) {
        const lenderAmounts = new Map<string, { minAmt: number; maxAmt: number }>();
        for (const p of prodRes.data) {
          const existing = lenderAmounts.get(p.lender_id);
          const pMin = p.min_amount ? Number(p.min_amount) : 0;
          const pMax = p.max_amount ? Number(p.max_amount) : Infinity;
          if (!existing) {
            lenderAmounts.set(p.lender_id, { minAmt: pMin, maxAmt: pMax });
          } else {
            existing.minAmt = Math.min(existing.minAmt, pMin);
            existing.maxAmt = Math.max(existing.maxAmt, pMax);
          }
        }
        filtered = filtered.filter(l => {
    ...
    const results = filtered.map(l => {
      const lines: string[] = [
        `## ${l.display_name}`,
        `**Type:** ${l.lender_type} | **State:** ${l.headquarters_state || 'N/A'}`,
      ...
      if (l.customers_praise && l.customers_praise.length > 0) lines.push(`**Customers praise:** ${l.customers_praise.slice(0, 3).join('; ')}`);
      if (l.customers_warn && l.customers_warn.length > 0) lines.push(`**Customers note:** ${l.customers_warn.slice(0, 2).join('; ')}`);
      lines.push(`**Website:** ${l.website_url || 'N/A'}`);
      lines.push(`**LenderWiki profile:** /lenders/${l.slug}`);
      lines.push('');
      return lines.join('\n');
    });
    ...
    return {
      content: [{
        type: "text" as const,
        text: `Found ${filtered.length} lender(s):\n\n${results.join('\n---\n\n')}\n\n_Data from LenderWiki. Last verified dates vary by lender._`,
      }],
    };
  }
);
...
server.tool(
  "get_lender_profile",
  "Get a detailed profile of a specific lender including products, eligibility requirements, fees, customer feedback, and regulatory status. Use the lender's slug (URL-friendly name) to look them up.",
  {
...
server.tool(
  "compare_lenders",
  "Compare two or more lenders side by side. Provides a comparison table of key metrics including rates, fees, eligibility, and customer feedback.",
  {
...
server.tool(
  "check_eligibility",
  "Check whether a person is likely eligible for a specific lender's products based on their financial profile. Returns a detailed eligibility assessment with match reasons and caveats.",
  {
    ...
    has_itin: z.boolean().optional().describe("Whether the applicant uses an ITIN instead of SSN"),
